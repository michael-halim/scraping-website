...
def get_every_product(driver, phone, address):
        try:
            ...
            for i in range(1, PAGES+1):
                ...
                url = f'https://www.dekoruma.com/brands/TEKA?page={i}'
                driver.get(url)
                ...
                for name,price,links,pic in zip(product_names,product_prices,product_links,product_pictures):
                    ...
                    productList.append({'name':product_name, 'pic':product_picture,
                                        ...
                                        'tags':['dapur'],'furnitureLocation':['dapur'],
                                        'isProduct':1 })
                    ...
                    for desc,add_desc in zip(product_descriptions,additional_descs_object):
                        ...
                        for count, add_desc in enumerate(additional_descs_object):
                            ...
                            if isNext[0] and isNext[1] == 'warna':
                                ...
                                product_color = replace_multiple_char(product_color, AERTEKA_CHAR_TO_REPLACE)

                            elif isNext[0] and isNext[1] == 'material':
                                material = tmp_additional_description.lower()
                                material = replace_multiple_char(material, AERTEKA_CHAR_TO_REPLACE)
                                material = material.strip()
                                material = replace_multiple_char(material, AERTEKA_MATERIAL_REPLACE)
                                material = material.strip()
                                material = replace_multiple_char(material, AERTEKA_HARD_REMOVE_MATERIAL)
                                material = material.strip()
                            ...
                            if count % 2 == 0 and count != 0 :
                                ...
                                tmp_additional_description = replace_multiple_char(tmp_additional_description, AERTEKA_CHAR_TO_REPLACE)
...
def main():
    ...
    print_help(var='RUNNING AER TEKA WEB SCRAPING....', title='AER TEKA WEB SCRAPING', username='MAIN', save_log_path=SAVE_LOG_PATH, log_filename=LOG_FILENAME)
    ...
    print_help(var='FINISHED AER TEKA WEB SCRAPING....', title='AER TEKA WEB SCRAPING', username='MAIN', save_log_path=SAVE_LOG_PATH, log_filename=LOG_FILENAME)
