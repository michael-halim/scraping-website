...
# # print(res.group(5))
# # print(res.group(6))
# a = 'Ellis Changing Table dilengkapi dengan enam laci untuk menyimpan baju-baju dan keperluan lainnya. Bagian atas bisa dilepas dan dijadikan lemari buffet. Dikirim jadi utuh tanpa perakitan. Ellis Changing TableP 100 cm L 48 cm T 82. 5 cm Harga Rp 2. 090. 000 (sudah termasuk busa Bahan kayu solid, laci kayu, mdf, cat ducoWarna Ivory (putih gading'
# res = re.search(r'P(?:anjang)?\s?([\d,.-]+)\s(?:cm|m)?(?:\s)?(?:L|Kedalaman)?\s([\d,.-]+)\s(?:cm|m)?(?:\s)?T(?:inggi)?\s?([\d,.-]+)\s(?:cm|m)?\b',a)
# print(res.group(1))
# print(res.group(2))
# print(res.group(3))
